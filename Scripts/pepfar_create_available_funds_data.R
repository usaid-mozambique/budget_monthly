...
pepfar_open_commitments <- blingr::clean_phoenix_open_commitments(open_commitments_data) |> 
    filter(`Program Area` == PROGRAM_AREA_FILTER,
           `Operating Unit` == OPERATING_UNIT_FILTER)
...
all_bi_acc_lines <- map(bi_acc_lines_all_files, test_create_history_bi_oblg_acc_lines, TRUE) |> 
    bind_rows() |> 
    dplyr::filter(`Funding Office Code` == FUNDING_OFFICE_CODE_FILTER,
                  `Avail for Subobl Amt` > AVAIL_FOR_SUBOBL_AMT_FILTER) |>

#write data
write_csv(all_bi_acc_lines, "Dataout/pepfar_all_bi_oblg_acc_lines.csv")
